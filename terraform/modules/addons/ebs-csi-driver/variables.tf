...
variable "addon_version" {
  ...
  validation {
    ...
    error_message = "Add-on version must be in format vX.Y.Z-eksbuild.N (e.g., v1.28.0-eksbuild.1)."
  }
}

variable "configuration_values" {
  description = "Configuration values for the EBS CSI Driver add-on (JSON string)"
  type        = map(any)
  default     = null
}
